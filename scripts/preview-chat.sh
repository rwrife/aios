...
exec docker run --rm --name aios-chat-windowed \
  ...
  -w /workspace "${AIOS_PREVIEW_IMAGE:-$IMAGE}" sh -ec '
    if ! command -v cmake >/dev/null; then
      apk add --no-cache build-base cmake ninja qt6-qtbase-dev qt6-qtdeclarative-dev qt6-qtmultimedia-dev qt6-qtmultimedia-gstreamer qt6-qtwebengine-dev python3 font-dejavu
    fi
