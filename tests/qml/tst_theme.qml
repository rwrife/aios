...
TestCase {
    ...
    function test_date_time_page(data) {
        ...
        var section = findChild(window, "settingsSections").itemAt(8)
        var setup = findChild(window, "launchSetup")
        verify(section.mapToItem(window.contentItem, 0, section.height).y <= setup.mapToItem(window.contentItem, 0, 0).y)
