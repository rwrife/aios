# Running AIOS: setup and reference
...
tool. Camera passthrough setup is documented in [the webcam guide](wsl-webcam.md#camera-inside-the-windowed-vm).
...
Network & Wi-Fi, Display, and Appearance. See [desktop settings](settings.md).
...
AI models also supports [ChatGPT subscription sign-in](chatgpt-subscription.md),
...
[`voice and sessions`](voice-and-sessions.md). Live sessions are ephemeral.
...
[browser tools](browser.md). The starter supports simple tool bootstrap but
...
[three stronger Qwen3 models](local-models.md) with tool capabilities,
...
[recovery and diagnostics](hardware-recovery.md). The VM launchers
