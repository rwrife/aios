...
The live root uses a tmpfs ceiling of 75% of RAM. The default development, voice,
and WebEngine packages exceed Alpine's default half-RAM limit. Live systems need
at least 6 GiB so package storage and the bundled local model both have working
headroom. The ceiling does not reserve memory up front and does not apply to
installed ext4 systems. Large models should use installed or mounted storage.
